...
router.get("/products/barcode/:barcode", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({ ...result.product, categoryName: result.categoryName || null, stockQuantity: Number(inv[0]?.qty ?? 0) });
});
...
router.get("/products", requireAuth, async (req, res): Promise<void> => {
  ...
  let query = db.select({ product: productsTable, categoryName: categoriesTable.name })
    .from(productsTable)
    .leftJoin(categoriesTable, eq(productsTable.categoryId, categoriesTable.id))
  ...
  const result = await Promise.all(products
    ...
    .map(async ({ product, categoryName }) => {
      ...
      return { ...product, categoryName: categoryName || null, stockQuantity };
    })
  );
...
router.post("/products", requireAuth, async (req, res): Promise<void> => {
  const orgId = req.organizationId!;
  const { name, price, description, sku, barcode, costPrice, categoryId, unit, taxRate, image, trackInventory, reorderPoint } = req.body;
  ...
  res.status(201).json({ ...product, categoryName: null, stockQuantity: 0 });
});
...
router.patch("/products/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  const fields = ["name","description","sku","barcode","price","costPrice","categoryId","unit","taxRate","image","trackInventory","reorderPoint","status"];
  ...
  const inv = await db.select({ qty: sum(inventoryTable.quantity) }).from(inventoryTable).where(eq(inventoryTable.productId, id));
  res.json({ ...product, categoryName: null, stockQuantity: Number(inv[0]?.qty ?? 0) });
});
